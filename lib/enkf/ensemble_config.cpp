...
bool ensemble_config_GEN_DATA_require_summary(const  ensemble_config_type * config) {
  bool ret = false;
  for(const auto& node_iter : config->config_nodes) {
    const enkf_config_node_type * config_node = node_iter.second;
...
enkf_config_node_type * ensemble_config_add_summary(ensemble_config_type * ensemble_config , const char * key , load_fail_type load_fail) {
  ...
  if (node_it != ensemble_config->config_nodes.end()) {
    ...
    summary_config_type * summary_config = (summary_config_type *)enkf_config_node_get_ref( config_node );
